  ...
  private def start(listenerEndpoints: Endpoints): Unit = synchronized {
    if (started) {
      ...
      try {
        ...
        if (sharedServerConfig.processRoles.contains(ProcessRole.BrokerRole) || sharedServerConfig.processRoles.contains(ProcessRole.BatchBrokerRole)) {
          brokerMetrics = BrokerServerMetrics(metrics)
        }
        if (sharedServerConfig.processRoles.contains(ProcessRole.ControllerRole) || sharedServerConfig.processRoles.contains(ProcessRole.BatchControllerRole)) {
